...
class SourceC(params: InclusiveCacheParameters) extends Module
{
  ...
  c.bits.size    := Mux(s3_req.blindmask_phase, UInt(params.offsetBits), UInt(params.offsetBits)) // FIXME: size hack
  c.bits.source  := s3_req.source
  c.bits.address := Mux(s3_req.blindmask_phase, params.expandBlindmaskAddr(s3_req.tag, s3_req.set, UInt(0)), params.expandDataAddr(s3_req.tag, s3_req.set, UInt(0)))
